...
const paymentDriverForType = (type: 'fiat_bank' | 'cards' | 'wps' | 'crypto') => {
  if (type === 'crypto') return 'crypto';
  if (type === 'cards') return 'stripe';
  return 'bank_transfer';
};

const mapGatewayCard = (gateway: any): PaymentGatewayCard => ({
  id: gateway.id,
  name: gateway.name,
  type: gateway.driver === 'bank_transfer' ? 'fiat_bank' : gateway.driver,
  currency: 'AED',
  feePercent: 0,
  settlementMode: gateway.driver === 'log' ? 'manual-log' : 'provider',
  apiKeyMasked: gateway.has_credentials ? 'stored-encrypted' : 'log-mode',
  status: gateway.is_active ? 'active' : 'disabled',
  lastTestMessage: gateway.last_test_message,
});
...
export const SuperAdminPage: React.FC = () => {
  ...
  const [paymentGateways, setPaymentGateways] = useState<PaymentGatewayCard[]>([]);
  const [paymentError, setPaymentError] = useState<string | null>(null);
  const [paymentBusyId, setPaymentBusyId] = useState<number | string | null>(null);
  ...
  const visiblePaymentGateways = paymentGateways.length > 0 ? paymentGateways : (globalSettings.paymentGatewaysList || []);

  const loadPaymentGateways = () => {
    adminApi.paymentGateways()
      .then((res: any) => {
        if (res.success) {
          setPaymentGateways(res.data.map(mapGatewayCard));
          setPaymentError(null);
        }
      })
      .catch((err) => setPaymentError(getApiError(err, 'Could not load live payment gateways. Showing local fallback data.')));
  };
  ...
  const handleAddPaymentGateway = async () => {
    ...
    setPaymentError(null);
    try {
      const res: any = await adminApi.createPaymentGateway({
        name: newGwName,
        driver: paymentDriverForType(newGwType),
        display_name: newGwName,
        credentials: newGwApiKey ? { api_key: newGwApiKey, currency: newGwCurrency, fee_percent: newGwFee, settlement: newGwSettlement } : undefined,
      });
      if (res.success) {
        setPaymentGateways((items) => [mapGatewayCard(res.data), ...items]);
      }
    } catch (err) {
      setPaymentError(getApiError(err, 'Could not save the payment gateway.'));
      return;
    }
    setNewGwName('');
    setNewGwApiKey('');
  ...
  const handleTogglePaymentGateway = async (gateway: PaymentGatewayCard) => {
    setPaymentBusyId(gateway.id);
    setPaymentError(null);
    try {
      const res: any = await adminApi.setPaymentGatewayActive(gateway.id, gateway.status !== 'active');
      if (res.success) {
        setPaymentGateways((items) => items.map((item) => item.id === gateway.id ? mapGatewayCard(res.data) : item));
      }
    } catch (err) {
      setPaymentError(getApiError(err, 'Could not update this payment gateway.'));
      if (paymentGateways.length === 0) togglePaymentGateway(String(gateway.id));
    } finally {
      setPaymentBusyId(null);
    }
  };

  const handleRemovePaymentGateway = async (gateway: PaymentGatewayCard) => {
    setPaymentBusyId(gateway.id);
    setPaymentError(null);
    try {
      const res: any = await adminApi.deletePaymentGateway(gateway.id);
      if (res.success) {
        setPaymentGateways((items) => items.filter((item) => item.id !== gateway.id));
      }
    } catch (err) {
      setPaymentError(getApiError(err, 'Could not remove this payment gateway.'));
      if (paymentGateways.length === 0) removePaymentGateway(String(gateway.id));
    } finally {
      setPaymentBusyId(null);
    }
  };

  const handleTestPaymentGateway = async (gateway: PaymentGatewayCard) => {
    setPaymentBusyId(gateway.id);
    setPaymentError(null);
    try {
      const res: any = await adminApi.testPaymentGateway(gateway.id);
      if (res.success) {
        setPaymentGateways((items) => items.map((item) => item.id === gateway.id ? mapGatewayCard(res.data) : item));
      }
    } catch (err) {
      setPaymentError(getApiError(err, 'Could not test this payment gateway.'));
    } finally {
      setPaymentBusyId(null);
    }
  };
    ...
    <div className="space-y-6 text-left font-sans max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 border border-[#E4EAF2] shadow-sm space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-gray-100">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[11px] font-mono bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-xl text-emerald-800 font-semibold">
              Gateways: {visiblePaymentGateways.filter(g => g.status === 'active').length} Active
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-slate-100 rounded-2xl">
          {[
            { id: 'payment', name: '💳 Payment Gateways & Rails', count: visiblePaymentGateways.length },
            { id: 'ai', name: '🧠 AI Engines & Vision', count: (globalSettings.aiProviders?.engines || []).length },
            { id: 'email', name: '✉️ Email & SMTP Relays' },
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                      <button
                        type="button"
                        onClick={() => handleTogglePaymentGateway(gw)}
                        disabled={paymentBusyId === gw.id}
                      ...
                      >
                        ...
                        <span>{paymentBusyId === gw.id ? 'Working...' : isActive ? 'Disable' : 'Enable'}</span>
                      </button>

                      <button
                        type="button"
                        onClick={() => handleTestPaymentGateway(gw)}
                        disabled={paymentBusyId === gw.id}
                        title="Test Gateway"
                        className="px-2 py-1.5 rounded-xl bg-blue-50 text-[#168BFF] hover:bg-blue-100 transition-colors cursor-pointer disabled:opacity-60"
                      >
                        Test
                      </button>
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemovePaymentGateway(gw)}
                        disabled={paymentBusyId === gw.id}
                        title="Remove Gateway"
                        className="p-1.5 rounded-xl text-red-500 hover:bg-red-50 transition-colors cursor-pointer disabled:opacity-60"
                      >
        ...
        {/* TAB 3: TRANSACTIONAL EMAIL (live, backed by the Laravel API) */}
        {activeApiTab === 'email' && <EmailSettingsPanel />}
