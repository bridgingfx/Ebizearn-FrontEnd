...
export const adminApi = {
  ...
  verificationQueue: (params?: { status?: string; search?: string }) =>
    api.get('/admin/verification-queue', { params }).then((r) => r.data as { success: boolean; data: TaskSubmission[]; meta?: unknown }),
  ...
  fraudAlerts: () => api.get('/admin/fraud-alerts').then((r) => r.data as { success: boolean; data: FraudEvent[]; meta?: unknown }),
  payouts: (params?: { status?: string }) =>
    api.get('/admin/payouts', { params }).then((r) => r.data as { success: boolean; data: WithdrawalRequest[]; meta?: unknown }),
  ...
  auditLogs: () => api.get('/admin/audit-logs').then((r) => r.data as { success: boolean; data: AuditLog[]; meta?: unknown }),
  users: (params?: { role?: string; search?: string }) =>
    api.get('/admin/users', { params }).then((r) => r.data as { success: boolean; data: User[]; meta?: unknown }),
  updateUserStatus: (userId: number | string, status: 'active' | 'suspended' | 'pending_verification') =>
    api.patch(`/admin/users/${userId}/status`, { status }).then((r) => r.data as { success: boolean; data: User }),
