...
export const AdminUsersPage: React.FC = () => {
  const { user } = useAuth();
  const { verifyUserKyc } = usePlatform();
  ...
  useEffect(() => {
    adminApi.users()
      .then((res) => {
        if (res.success) {
          setUsers(res.data.map((apiUser: ApiUser) => {
            const isSuspended = apiUser.status === 'suspended';
            const isPending = apiUser.status === 'pending_verification';
            const balanceCents = apiUser.wallet?.available_balance_cents || 0;
            return {
              id: `USR-${apiUser.id}`,
              rawId: apiUser.id,
              name: apiUser.business?.company_name ? `${apiUser.business.company_name} (${apiUser.name})` : apiUser.name,
              email: apiUser.email,
              role: apiUser.role === 'business' ? 'Business' : apiUser.role === 'superadmin' ? 'Super Admin' : apiUser.role === 'admin' ? 'Moderator' : 'Contributor',
              tier: apiUser.profile?.contributor_level ? `Level ${apiUser.profile.contributor_level}` : apiUser.business?.status || 'Account',
              status: isSuspended ? 'Banned' : isPending ? 'Pending' : 'Active',
              statusColor: isSuspended ? 'bg-red-50 text-red-700 border-red-200' : isPending ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-[#16B364] border-emerald-200',
              balance: money(balanceCents),
              lifetimeEarnings: apiUser.wallet ? money(apiUser.wallet.lifetime_earnings_cents || 0) : 'n/a',
              kyc: apiUser.profile?.kyc_status === 'verified' ? 'Emirates ID Verified' : apiUser.profile?.kyc_status === 'rejected' ? 'Rejected' : 'Pending Review',
              riskScore: `Score ${(apiUser.profile?.fraud_score || 0).toFixed(2)}`,
              country: apiUser.profile?.country_code || 'n/a',
              workLocation: apiUser.profile?.city || apiUser.profile?.country_code || 'Not provided',
              ipAddress: 'n/a',
              isp: 'n/a',
              device: 'n/a',
              geofenceStatus: 'Not captured',
              joined: 'Live user',
            };
          }));
          setLoadError('');
  ...
  const handleToggleBan = async (userId: string) => {
    ...
    const nextStatus = target.status === 'Banned' ? 'active' : 'suspended';
    try {
      await adminApi.updateUserStatus(target.rawId, nextStatus);
      setUsers(
        users.map((u) => {
          if (u.id === userId) {
    ...
    <div className="space-y-6 text-left font-sans max-w-7xl mx-auto">
      ...
      {loadError && (
        <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-xs font-bold text-amber-900">
          Live user directory could not load or update: {loadError}
        </div>
      )}
              ...
              >
                <div className="flex items-start justify-between gap-2 pb-3 border-b border-white/10">
                  <div className="flex items-center gap-2.5">
                    <UserAvatar src={k.avatar} name={k.name} className="ring-2 ring-cyan-400/40" />
                    <div>
