...
import { mapCampaignForUi, mapSubmissionForUi } from '../../utils/apiMappers';
import { getApiError } from '../../api/client';

export const BusinessDashboardPage: React.FC = () => {
  const { campaigns: fallbackCampaigns } = usePlatform();
    ...
    businessApi.dashboard()
      .then((res) => {
        if (res.success) {
          ...
          setLiveCampaigns((res.data.active_campaigns_list || []).map(mapCampaignForUi));
          setLoadError('');
        }
      })
  ...
  const campaigns = liveCampaigns.length > 0 ? liveCampaigns : fallbackCampaigns;
    ...
    <div className="space-y-6 text-left font-sans">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="p-4 sm:p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs">
          ...
          <span className="text-2xl sm:text-3xl font-black text-[#101828] mt-1 block">{metrics?.active_campaigns ?? campaigns.length}</span>
        ...
        <div className="p-4 sm:p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs">
          ...
          <span className="text-2xl sm:text-3xl font-black text-[#101828] mt-1 block">AED {((metrics?.spent_budget_cents ?? 428000) / 100).toFixed(2)}</span>
          <span className="text-[11px] text-[#18B76A] font-bold flex items-center gap-0.5 mt-1">
        ...
        <div className="p-4 sm:p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs">
          ...
          <span className="text-2xl sm:text-3xl font-black text-[#101828] mt-1 block">{metrics?.verified_tasks ?? '8,943'}</span>
          <span className="text-[11px] text-[#18B76A] font-bold flex items-center gap-0.5 mt-1">
        ...
        <div className="col-span-2 md:col-span-1 p-4 sm:p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs">
          ...
          <span className="text-2xl sm:text-3xl font-black text-[#101828] mt-1 block">AED {((metrics?.average_cost_cents ?? 48) / 100).toFixed(2)}</span>
          <span className="text-[11px] text-[#18B76A] font-bold flex items-center gap-0.5 mt-1">
