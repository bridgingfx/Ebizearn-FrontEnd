    ...
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        className={`flex items-center gap-2 pl-2 pr-2.5 py-1.5 rounded-full border transition-all min-h-[40px] ${
          dark
            ...
            : 'border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-800 shadow-xs'
        }`}
      >
        <RegionFlag region={current} />
        <span className={`text-xs font-black tracking-wide ${dark ? 'text-white' : 'text-slate-900'}`}>
          {current.currency}
        </span>
        <span className={`h-3.5 w-px ${dark ? 'bg-white/20' : 'bg-slate-200'}`} />
        <span className={`text-[11px] font-bold ${dark ? 'text-slate-300' : 'text-slate-500'}`}>
          {current.code === 'AE' ? 'UAE' : current.name.split(' ')[0]}
        </span>
        <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ? 'rotate-180' : ''} ${dark ? 'text-slate-400' : 'text-slate-400'}`} />
      </button>
      ...
      {open && (
        <div
          ...
          className="absolute right-0 mt-2 w-64 bg-white rounded-2xl border border-slate-200 shadow-2xl shadow-slate-900/10 overflow-hidden z-50 animate-in fade-in"
        >
          <p className="px-4 pt-3 pb-1 text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    <span className="block text-sm font-bold text-slate-800 truncate">{r.name}</span>
                    <span className="block text-[11px] text-slate-400">Currency: {r.currency}</span>
                  </span>
