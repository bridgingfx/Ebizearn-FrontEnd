...
export const ProviderManager: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {notice && (
        <div className={`px-4 py-2.5 rounded-xl text-xs font-semibold ${notice.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
          {notice.text}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="p-6 rounded-2xl border border-dashed border-gray-300 text-center text-xs text-gray-500">
          No providers yet. Add one to start sending real emails.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {providers.map((p) => (
            <div key={p.id} className={`p-4 rounded-2xl border space-y-3 ${p.is_active ? 'bg-white border-emerald-300' : 'bg-white border-slate-200'}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-blue-100 text-blue-800">{DRIVER_LABELS[p.driver]}</span>
                  <h4 className="text-sm font-black text-gray-900 mt-1.5 truncate">{p.name}</h4>
                  <p className="text-[11px] text-slate-500 font-mono truncate">{p.from_name} &lt;{p.from_email}&gt;</p>
                </div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${p.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
              ...
              <div className="text-[11px] flex items-start gap-1.5">
                ...
                {p.status === 'failed' && <XCircle className="w-3.5 h-3.5 text-red-600 shrink-0 mt-0.5" />}
              ...
              <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center gap-2">
                <button type="button" onClick={() => toggleActive(p)} disabled={busyId === p.id}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer ${p.is_active ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
                ...
                <button type="button" onClick={() => setTestingId(testingId === p.id ? null : p.id)}
                  className="px-3 py-1.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 hover:bg-blue-100 flex items-center gap-1.5 cursor-pointer">
                  <Send className="w-3.5 h-3.5" />Test
                </button>
                <button type="button" onClick={() => openForm(p)}
                  className="px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 text-slate-700 hover:bg-slate-200 flex items-center gap-1.5 cursor-pointer">
                  <Pencil className="w-3.5 h-3.5" />Edit
                </button>
                <button type="button" onClick={() => remove(p)} disabled={busyId === p.id} title="Delete"
                  className="ml-auto p-1.5 rounded-xl text-red-500 hover:bg-red-50 cursor-pointer">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <form onSubmit={save} className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-black text-gray-900">{editing === 'new' ? 'Add email provider' : 'Edit email provider'}</h3>
              <button type="button" onClick={() => setEditing(null)} className="p-1 text-gray-500 hover:text-gray-900 cursor-pointer"><X className="w-4 h-4" /></button>
            </div>

            <label className="block text-[11px] font-bold text-gray-600">Name
            ...
            <label className="block text-[11px] font-bold text-gray-600">Driver
            ...
            {(driver === 'smtp' || driver === 'mailgun') && (
              <label className="block text-[11px] font-bold text-gray-600">{driver === 'smtp' ? 'SMTP host' : 'Mailgun domain'}
            ...
            {driver === 'smtp' && (
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-[11px] font-bold text-gray-600">Port
                ...
                <label className="block text-[11px] font-bold text-gray-600">Encryption
            ...
            {(driver === 'smtp' || driver === 'ses') && (
              <label className="block text-[11px] font-bold text-gray-600">Username
            ...
            {(driver === 'ses' || driver === 'mailgun') && (
              <label className="block text-[11px] font-bold text-gray-600">{driver === 'ses' ? 'AWS region' : 'Region'}
            ...
            {driver !== 'log' && (
              <label className="block text-[11px] font-bold text-gray-600">{SECRET_LABELS[driver]}
                <input
                  type="password"
                  value={form.secret ?? ''}
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block text-[11px] font-bold text-gray-600">From email
              ...
              <label className="block text-[11px] font-bold text-gray-600">From name
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setEditing(null)} className="px-4 py-2 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-100 cursor-pointer">Cancel</button>
