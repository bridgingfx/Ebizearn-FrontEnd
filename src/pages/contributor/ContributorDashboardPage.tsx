...
export const ContributorDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    tasksApi.contributorDashboard()
      .then((res) => {
        if (res.success) {
          setLiveStats(res.data.stats);
          setLiveTasks((res.data.recommended_tasks || []).map(mapTaskForUi));
          setLoadError('');
        }
      })
  ...
  const dashboardTasks = liveTasks.length > 0 ? liveTasks : platformTasks;
  const totalCompleted = liveStats?.completed_tasks_count ?? ((user?.profile?.completed_tasks_count || 12) + approvedCount);
  const totalAvailable = dashboardTasks.length;
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs flex items-center justify-between">
          <div>
            ...
            <span className="text-2xl sm:text-3xl font-black text-[#101828] mt-1 block">
              {(liveStats?.pending_tasks_count ?? pendingCount) > 0 ? `${liveStats?.pending_tasks_count ?? pendingCount} Tasks` : `AED ${((liveStats?.pending_balance_cents ?? user?.wallet?.pending_balance_cents ?? 450) / 100).toFixed(2)}`}
            </span>
        ...
        <div className="p-5 rounded-3xl bg-white border border-[#E7ECF3] shadow-xs flex items-center justify-between">
          <div>
            ...
            <span className="text-2xl sm:text-3xl font-black text-[#168BFF] mt-1 block">
              AED {((liveStats?.available_balance_cents ?? user?.wallet?.available_balance_cents ?? 2840) / 100).toFixed(2)}
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {loadError && (
          <div className="lg:col-span-12 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800">
            Live dashboard data could not load: {loadError}
          </div>
        )}
