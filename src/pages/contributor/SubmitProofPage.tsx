...
export const SubmitProofPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    tasksApi.get(id)
      .then((res) => {
        if (res.success) setLiveTask(mapTaskForUi(res.data));
      })
      .catch(() => undefined);
  }, [id]);

  const currentTask = liveTask || tasks.find((t) => t.id === Number(id)) || tasks[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await tasksApi.submit(id as string, {
      ...
      if (res.success) {
        ...
        if (res.data?.ai_precheck) {
          setAiAnalysis({
            confidence: res.data.ai_precheck.confidence_score,
            decision: res.data.ai_precheck.suggested_decision,
            summary: res.data.ai_precheck.summary,
          });
        }
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 text-left font-sans">
      ...
      {submitError && (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-2 text-xs font-semibold text-red-700">
          {submitError}
        </div>
      )}
