  ...
  const services = [
    {
      ...
      status: 'Healthy',
      latency: health?.status === 'operational' ? 'Live' : 'n/a',
      uptime: '99.99%',
      icon: Server,
    },
    {
    ...
    },
    {
      ...
      status: health?.queue === 'idle' ? 'Healthy' : 'Busy',
      latency: '1ms',
      uptime: '100%',
      icon: Radio,
    },
  ...
  return (
    <div className="space-y-6 text-left font-sans max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {loadError && (
          <div className="sm:col-span-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800">
            Live health data could not load: {loadError}
          </div>
        )}
      ...
      {health && (
        <div className="rounded-3xl bg-white border border-[#E7ECF3] p-5 shadow-xs grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
          <div><span className="text-gray-400 block">PHP</span><strong>{health.php_version}</strong></div>
          <div><span className="text-gray-400 block">Laravel</span><strong>{health.laravel_version}</strong></div>
          <div><span className="text-gray-400 block">Cache</span><strong>{health.cache}</strong></div>
          <div><span className="text-gray-400 block">Storage</span><strong>{health.storage}</strong></div>
        </div>
      )}
