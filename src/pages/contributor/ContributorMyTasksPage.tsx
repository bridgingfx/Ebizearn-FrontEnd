...
export const ContributorMyTasksPage: React.FC = () => {
  ...
  const submissionSource = liveSubmissions.length > 0 ? liveSubmissions : platformSubmissions;
  const submissions: Submission[] = submissionSource.map((ps) => {
    ...
    <div className="space-y-6 text-left font-sans">
      ...
      {loadError && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-800">
          Live task history could not load: {loadError}
        </div>
      )}
