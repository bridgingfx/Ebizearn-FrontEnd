  ...
  return (
    <div className="space-y-5 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-[#101828]">My Tasks</h1>
      ...
      {/* Filters */}
      <div className="bg-white rounded-2xl p-3 sm:p-4 border border-[#E7ECF3] space-y-3">
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 ${
                filter === tab.key ? 'bg-[#07182F] text-white' : 'text-gray-600 hover:bg-gray-100'
              }`}
            >
              <span>{tab.label}</span>
              <span className={`text-[10px] px-1.5 py-px rounded-full ${filter === tab.key ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600'}`}>
                {countFor(tab.key)}
              </span>
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((s) => {
            const meta = STATUS_META[s.status] || { label: s.status.replace(/_/g, ' '), className: 'bg-gray-100 text-gray-600 border-gray-200', icon: Clock };
            const Icon = meta.icon;
            const task = s.uiTask;
            return (
              <div key={s.id} className="bg-white rounded-2xl p-4 sm:p-5 border border-[#E7ECF3] shadow-xs">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      ...
                      {task && (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-gray-100 text-gray-600">{task.platform}</span>
                      )}
                      <span className="text-[10px] text-gray-400">
                        Submitted {new Date(s.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                      </span>
                    </div>
                    <h3 className="text-sm font-black text-gray-900">{task?.title || `Task #${s.task_id}`}</h3>
                    {task && <p className="text-[11px] text-gray-500">{task.brandName}</p>}
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-700 text-xs font-bold transition-colors"
                  >
                    <Eye className="w-3.5 h-3.5 text-gray-400" /> View proof
                  </button>
                  {task && (
                    <Link
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Submission #{selected.id}</p>
                <h3 className="text-base font-black text-gray-900">{selected.uiTask?.title || `Task #${selected.task_id}`}</h3>
              </div>
              <button type="button" onClick={() => setSelected(null)} className="p-1.5 rounded-lg hover:bg-gray-100" aria-label="Close">
            ...
            {selected.files && selected.files.length > 0 && (
              <div className="space-y-2">
                {selected.files.map((f) => (
                  <div key={f.id} className="rounded-2xl overflow-hidden border border-gray-200 bg-gray-900 flex items-center justify-center">
            ...
            {selected.proof_data_json?.url && (
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-200 text-xs">
                <span className="text-gray-500 font-bold block mb-0.5">Proof URL</span>
            ...
            {selected.proof_data_json?.text_answer && (
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-200 text-xs">
                <span className="text-gray-500 font-bold block mb-0.5">Answer</span>
                <p className="text-gray-700 whitespace-pre-wrap">{selected.proof_data_json.text_answer}</p>
              </div>
            )}

            {selected.proof_data_json?.note && (
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-200 text-xs">
                <span className="text-gray-500 font-bold block mb-0.5">Note</span>
                <p className="text-gray-700 italic">{selected.proof_data_json.note}</p>
              </div>
            )}
